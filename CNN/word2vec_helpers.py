...
def embedding_sentences(sentences, embedding_size = 128, window = 5, min_count = 5, file_to_load = None, file_to_save = None):
    ...
    print('dim....', embeddingDim)
    embeddingUnknown = [0 for i in range(embeddingDim)]
    for sentence in sentences:
        this_vector = []
        for word in sentence:
            if word in w2vModel.wv.vocab:
                this_vector.append(np.array(w2vModel[word]))
            else:
                this_vector.append(np.array(embeddingUnknown))
        all_vectors.append(np.array(this_vector))
    print('all vectors len...', len(all_vectors))
    print('vector len...', len(all_vectors[0]))
    print('vector len...', len(all_vectors[0][0]))
    return all_vectors
