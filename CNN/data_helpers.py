...
def load_data_and_labels(input_text_file, input_label_file, num_labels):
    x_brand, x_text = read_and_clean_zh_file_2(input_text_file)
    y = None if not os.path.exists(input_label_file) else map(int, list(open(input_label_file, "r").readlines()))
    return (x_brand, x_text, y)

def load_positive_negative_data_files(positive_data_file, negative_data_file, natural_data_file):
    """
    ...
    natural_examples = read_and_clean_zh_file(natural_data_file)
    # Combine data
    # 
    # print('positive_examples', positive_examples)
    # print('negative_examples,,,', negative_examples)
    x_text = positive_examples + negative_examples + natural_examples
    # Generate labels
    positive_labels = [[0, 0, 1] for _ in positive_examples]
    negative_labels = [[1, 0, 0] for _ in negative_examples]
    natural_labels = [[0, 1, 0] for _ in natural_examples]
    y = np.concatenate([positive_labels, negative_labels, natural_labels], 0)
    return [x_text, y]
...
def batch_iter(data, batch_size, num_epochs, shuffle=True):
    ...
    for batch_num in range(num_batches_per_epoch):
        ...
        print(type(shuffled_data))
        print(type(shuffled_data[0]))
        print(shuffled_data[0].shape)
        yield np.array(shuffled_data[start_idx : end_idx])
...
def read_and_clean_zh_file(input_file, output_cleaned_file = None):
    lines = list(open(input_file, "r", encoding='gbk').readlines())
    # print(lines)
    # x_brand = [line.strip().split(',')[1] for line in lines]
    lines = [clean_str(seperate_line(line.strip())) for line in lines]
    # print('')
    # print(lines)
...
def read_and_clean_zh_file_2(input_file, output_cleaned_file = None):
    lines = list(open(input_file, "r", encoding='gbk').readlines())
    # print(lines)
    x_brand = [line.strip().split(',')[1] for line in lines]
    lines = [clean_str(seperate_line(line.strip().split(',')[2])) for line in lines]
    # print('')
    # print(lines)
    if output_cleaned_file is not None:
        with open(output_cleaned_file, 'w') as f:
