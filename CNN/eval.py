...
# Data Parameters
tf.flags.DEFINE_string("input_text_file", "/Users/dgy/PycharmProjects/submission/model/15_brands_unlabel.csv", "Test text data source to evaluate.")
tf.flags.DEFINE_string("input_label_file", "", "Label file for test text data source.")

# Eval Parameters
tf.flags.DEFINE_integer("batch_size", 64, "Batch Size (default: 64)")
tf.flags.DEFINE_string("checkpoint_dir", "runs/1565523912/checkpoints/", "Checkpoint directory from training run")
...
# Load data
if FLAGS.eval_train:
    x_brand, x_raw, y_test = data_helpers.load_data_and_labels(FLAGS.input_text_file, FLAGS.input_label_file, num_labels)
else:
...
print('sentences.......2....', len(sentences))
print('sentences.......2....', len(sentences[0]))
    ...
    with sess.as_default():
        ...
        for x_test_batch in batches:
            ...
            try:
            	...
            	batch_predictions = np.zeros((x_test_batch.shape[0]))
...
predictions_human_readable = np.column_stack((x_brand, np.array([text.replace(' ', '') for text in x_raw]), all_predictions))
