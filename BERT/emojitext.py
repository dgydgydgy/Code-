...
class InputExample(object):
    """A single training/test example for emojiTextple sequence classification."""
...
class emojiTextProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        file_path = os.path.join(data_dir, 'b_train.csv')
    ...
    def get_dev_examples(self, data_dir):
        file_path = os.path.join(data_dir, 'b_val.csv')
    ...
    def get_test_examples(self, data_dir):
        file_path = os.path.join(data_dir, 'b_test.csv')
...
class BertemojiText:
    ...
    def __init__(self, batch_size=args.batch_size):
        ...
        self.estimator = None
        self.processor = emojiTextProcessor()
    ...
    def model_fn_builder(self, bert_config, num_labels, init_checkpoint, learning_rate,
        ...
        def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
            ...
            (total_loss, per_example_loss, logits, probabilities) = BertemojiText.create_model(
    ...
    def _truncate_seq_pair(self, tokens_a, tokens_b, max_length):
        ...
        # This is a emojiTextple heuristic which will always truncate the longer sequence
...
if __name__ == '__main__':
    emojiText = BertemojiText()
    ### each line represent train, evaluation and predict
    # emojiText.set_mode(tf.estimator.ModeKeys.TRAIN)
    # emojiText.train()
    # emojiText.set_mode(tf.estimator.ModeKeys.EVAL)
    # emojiText.eval()

    emojiText.set_mode(tf.estimator.ModeKeys.PREDICT)
    while True:
        print("Please enter the text for prediction:")
        sentence1 = input('')
        predict = emojiText.predict(sentence1, "0")
        if predict[0][0]>0.7:
            print(f'The result of the prediction：Negative，probability：{predict[0][0]}')
        else:
            print(f'The result of the prediction：Positive，probability：{predict[0][1]}')
...
#         with open("hermes.csv","r",encoding="utf-8",errors='ignore') as review_file:
#             review_file.readline()
#             lines=review_file.readlines()
#             f=open("/Users/dgy/PycharmProjects/emoji_text/hermes.csv","a")
#             f.write("用户,时间,评论,情感,置信度,正面情感概率\n")
#             linecnt=0
...
#                 review=line[1]
#                 print(review)
#                 precit=emojiText.predict(review,"")
#                 try:
#                     if predict[0][0]>0.8:
#                         emotion=0
...
#                         np.sqrt(predict[0][1]*predict[0][0]),predict[0][1])
#                     print(result)
#                     f.write(result)
#                 except:
#                     time.sleep(0.5)
#                 linecnt+=1
#                 if linecnt%100==0:
#                     print("进度{}/{},{}%".format(linecnt,linelength,linecnt/linelength*100))
